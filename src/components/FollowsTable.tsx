...
const FollowsTable: React.FC<FollowsTableProps> = ({ userData, followData }) => {
        ...
        <div className="flex gap-2 flex-wrap">
          {(Object.keys(tabLabels) as TabKey[]).map(tab => (
            <button
              ...
              className={`px-4 py-2 rounded-full text-sm font-semibold transition-colors ${activeTab===tab ? 'bg-purple-600 text-white shadow' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
            >
              {tabLabels[tab]} <span className="ml-1 text-xs opacity-80">{dataMap[tab].length}</span>
            </button>
          ))}
        </div>
      ...
      <div className="overflow-auto border rounded-xl">
        <table className="min-w-full text-sm">
          <thead className="bg-gray-50">
            <tr>
              <Th>アイコン</Th>
              <SortableTh active={sortKey==='nickname'} dir={sortDir} onClick={()=>handleSort('nickname')}>ユーザー</SortableTh>
              <SortableTh active={sortKey==='follower_count'} dir={sortDir} onClick={()=>handleSort('follower_count')}>フォロワー数</SortableTh>
              <SortableTh active={sortKey==='following_count'} dir={sortDir} onClick={()=>handleSort('following_count')}>フォロー数</SortableTh>
              <Th>種別</Th>
            </tr>
          </thead>
          <tbody>
            {filtered.length === 0 && (
              <tr>
                <td colSpan={5} className="text-center py-10 text-gray-500">データがありません</td>
              </tr>
            )}
            {filtered.map(u => (
              <tr key={u.id} className="border-t hover:bg-purple-50/50">
                ...
                <td className="p-2 text-right tabular-nums">{u.follower_count?.toLocaleString?.()}</td>
                <td className="p-2 text-right tabular-nums">{u.following_count?.toLocaleString?.()}</td>
                <td className="p-2 text-xs">
                  {followData.mutualFollows.some(m=>m.id===u.id) ? (
                    <Badge color="blue">相互</Badge>
                  ) : followData.followers.some(f=>f.id===u.id) && followData.followings.some(f=>f.id===u.id) ? (
                    <Badge color="blue">相互</Badge>
                  ) : activeTab==='followers' ? (
...
const SortableTh: React.FC<{children: React.ReactNode; active: boolean; dir: 'asc'|'desc'; onClick: ()=>void}> = ({ children, active, dir, onClick }) => (
  <th
    onClick={onClick}
    className={`px-3 py-2 text-left text-[11px] font-semibold tracking-wider uppercase cursor-pointer select-none group ${active? 'text-purple-600':'text-gray-500 hover:text-gray-700'}`}
  >
    <span className="inline-flex items-center gap-1">
      {children}
      <span className="text-[9px] opacity-70 group-hover:opacity-100">{active ? (dir==='asc' ? '▲' : '▼') : '⇅'}</span>
    </span>
  </th>
);

const Badge: React.FC<{children: React.ReactNode; color?: 'red'|'teal'|'blue'|'gray'}> = ({ children, color='gray' }) => {
  ...
  return <span className={`border text-[10px] font-semibold px-2 py-1 rounded-full ${colorMap[color]}`}>{children}</span>;
};
