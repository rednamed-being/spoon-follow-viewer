...
export default function FollowVisualizer({ userData, followData, }) {
    ...
    useEffect(() => {
        const handleMouseMove = (e) => {
            ...
            // ノード位置を更新
            setNodes((prevNodes) => prevNodes.map((node) => node.id === dragState.nodeId
    ...
    const initializeNodes = () => {
        ...
        // より美しい円形配置のための計算
        const maxRadius = Math.min(containerRect.width, containerRect.height) / 2 - 80; // マージンを考慮
        ...
        const nonMutualFollowings = followData.followings.filter((following) => !followData.mutualFollows.some((m) => m.id === following.id));
        nonMutualFollowings.forEach((user, index) => {
            // 内側の円も上から開始し、外側とずらして配置
            const baseAngle = (index / nonMutualFollowings.length) * 2 * Math.PI;
            const angle = baseAngle - Math.PI / 2 + Math.PI / nonMutualFollowings.length; // 少しオフセット
    ...
    const renderGraph = () => {
        ...
        if (centerNode) {
            nodes.forEach((node) => {
                if (node.type !== "center") {
                    const line = createConnectionLine(centerNode.x, centerNode.y, node.x, node.y, node.type);
        ...
        // ノードを描画
        nodes.forEach((node) => {
            const element = node.type === "center"
                ? createCenterUserElement(node.user, node.id)
                : createUserElement(node.user, node.type, node.id);
    ...
    const addDragEvents = (element, nodeId) => {
    ...
    const createCenterUserElement = (user, nodeId) => {
    ...
    const createUserElement = (user, type, nodeId) => {
      ...
      <div class="text-center max-w-20">
        <div class="text-xs font-medium text-gray-800 truncate">${escapeHtml(user.nickname)}</div>
    ...
    const createConnectionLine = (x1, y1, x2, y2, type) => {
    ...
    return (_jsxs("div", { className: "bg-white rounded-2xl shadow-2xl p-8", children: [_jsxs("div", { className: "flex justify-center gap-8 mb-6 flex-wrap", children: [_jsxs("div", { className: "flex items-center gap-2", children: [_jsx("div", { className: "w-5 h-5 rounded-full bg-red-500" }), _jsx("span", { className: "text-sm text-gray-600", children: "\u30D5\u30A9\u30ED\u30EF\u30FC" })] }), _jsxs("div", { className: "flex items-center gap-2", children: [_jsx("div", { className: "w-5 h-5 rounded-full bg-teal-500" }), _jsx("span", { className: "text-sm text-gray-600", children: "\u30D5\u30A9\u30ED\u30FC\u4E2D" })] }), _jsxs("div", { className: "flex items-center gap-2", children: [_jsx("div", { className: "w-5 h-5 rounded-full bg-blue-500" }), _jsx("span", { className: "text-sm text-gray-600", children: "\u76F8\u4E92\u30D5\u30A9\u30ED\u30FC" })] }), _jsx("div", { className: "flex items-center gap-2", children: _jsx("span", { className: "text-sm text-gray-500", children: "\uD83D\uDCA1 \u30A2\u30A4\u30B3\u30F3\u3092\u30C9\u30E9\u30C3\u30B0\u3057\u3066\u79FB\u52D5\u3067\u304D\u307E\u3059" }) })] }), _jsx("div", { ref: containerRef, className: "relative w-full h-[500px] md:h-[700px] border-2 border-dashed border-gray-200 rounded-xl overflow-hidden bg-gradient-to-br from-gray-50 to-white", style: {
                    userSelect: "none",
                    aspectRatio: "1/1",
                    maxWidth: "700px",
                    margin: "0 auto",
